...
TODO/IMPROVE:
    * enforce the same size of some input arguments using parametric types =#
    ...
    export fem_prismatic_twin_girder, fem_general_twin_girder
    export fem_prismatic_single_girder, fem_general_single_girder
    export bending_moments_twin, bending_moments

    using LinearAlgebra: norm, triu, tril
    using Plots

    #= include(abspath(joinpath("continuous_girder", "analysis", "Configs.jl")))
    using .Configs: ANALYSIS_DIR, FEM_DIR =#

    include("Utils.jl")
    using .Utils: linear_index
