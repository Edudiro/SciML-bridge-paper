...
export linterp_even, linear_index, relative

using Plots: Subplot, xlims, ylims
...
```
"""
function relative(p::Subplot, rx, ry)
    x_lims = xlims(p)
    y_lims = ylims(p)
    return x_lims[1] + rx * (x_lims[2] - x_lims[1]), y_lims[1] +
        ry * (y_lims[2] - y_lims[1])
end
